...
if tmux has-session -t "$SESSION" 2>/dev/null; then
  ...
  tmux new-session -d -s "$SESSION" -x 220 -y 50 \
    ...
    -e "TZ=$TZ"
  tmux send-keys -t "$SESSION" "cd ${SCRIPT_DIR} && ${CLAUDE_BIN} -p "Hey, are there anything I should know now? Reply via Discord channel 1486128557444042883." --channels plugin:discord@claude-plugins-official --dangerously-skip-permissions" Enter
