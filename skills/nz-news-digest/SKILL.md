...
使用 `date` 指令獲取紐西蘭時間（Pacific/Auckland）：
- `NZT_DATE`: 展示用日期 (e.g., Monday 02 May 2026)
- `NZT_ISO`: ISO 格式日期 (YYYY-MM-DD)
- `NZT_MONTH`: 年月格式 (YYYY-MM)

---

### 步驟 2：擷取 RSS Feeds

使用可用之網頁擷取工具（如 `web_extract` 或 `fetch_external`）**依序**抓取以下 4 個 RSS feed。每個 feed 若失敗則跳過，繼續下一個。
...
將摘要以 Markdown 格式存入：
`/vault/07-NZ-News/{NZT_MONTH}/{NZT_ISO}.md`
...
**執行細節**：
- 確保目錄 `/vault/07-NZ-News/{NZT_MONTH}` 存在。
- 使用檔案寫入工具執行寫入。若同日已有檔案，直接覆寫（冪等）。
